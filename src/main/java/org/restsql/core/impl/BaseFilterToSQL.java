...
public class BaseFilterToSQL implements MySqlVisitor {
	...
	/** filter factory */
	protected static FilterFactory filterFactory = CommonFactoryFinder
	...
	/**
	 * the geometry descriptor corresponding to the current binary spatial
	 * filter being encoded
	 */
	...
	/**
	 * The dimension corresponding to the current binary spatial filter being
	 * encoded
	 */
	protected Integer currentDimension;

	/**
	 * inline flag, controlling whether "WHERE" will prefix the SQL encoded
	 * filter
	 */
	 ...
	 * Default constructor
	 */
	public BaseFilterToSQL() {
	}

	public BaseFilterToSQL(Writer out) {
		this.out = out;
	}
	 ...
	 * 
	 * @throws OpenGISFilterToOpenGISFilterToSQLEncoderException
	 ...
	 * Equivalent to:
	 * 
	 * StringWriter out = new StringWriter(); new
	 * FilterToSQL(out).encode(filter); out.getBuffer().toString();
	 * 
	 * @param filter
	 ...
	 * wildcards for the Like Filter: . for multi and .? for single. And
	 * replaces them with the SQL % and _, respectively.
	 * 
	 * @param filter
	...
	public Object visit(PropertyIsLike filter, Object extraData) {
		...
		AttributeDescriptor ad = (AttributeDescriptor) att
				.evaluate(featureType);
		if (ad != null
				&& Date.class.isAssignableFrom(ad.getType().getBinding())) {
			literal += multi;
		}

		String pattern = LikeFilterImpl.convertToSQL92(esc, multi, single,
				matchCase, literal);
	...
	public Object visit(Not filter, Object extraData) {
		try {
			if (filter.getFilter() instanceof PropertyIsNull) {
				Expression expr = ((PropertyIsNull) filter.getFilter())
						.getExpression();
				expr.accept(this, extraData);
	 ...
	 * 
	 * DJB: note, postgis overwrites this implementation because of the way null
	 * is handled. This is for <PropertyIsNull> filters and <PropertyIsEqual>
	 * filters are handled. They will come here with "property = null". NOTE:
	 * SELECT * FROM
	 * <table>
	 * WHERE <column> isnull; -- postgresql SELECT * FROM
	 * <table>
	 * WHERE isnull(<column>); -- oracle???
	 * 
	 * @param filter
	 ...
	 *             for io exception with writer
	 */
	protected void visitBinaryComparisonOperator(
			BinaryComparisonOperator filter, Object extraData)
			throws RuntimeException {
		...
		if (left instanceof PropertyName) {
			...
			AttributeDescriptor attType = (AttributeDescriptor) left
					.evaluate(featureType);
			if (attType != null) {
		...
		if (right instanceof PropertyName) {
			AttributeDescriptor attType = (AttributeDescriptor) right
					.evaluate(featureType);
			if (attType != null) {
		...
		try {
			if (matchCase) {
				...
				FunctionImpl f = new FunctionImpl() {
					{
						functionName = new FunctionNameImpl("lower", parameter(
								"lowercase", String.class), parameter("string",
								String.class));
					}
				};
				f.setName("lower");
	...
	/*
	 * returns the return type of the function, or null if it could not be
	 * determined or is simply of return type Object.class
	 */
	Class getFunctionReturnType(Function f) {
		Class clazz = Object.class;
		if (f.getFunctionName() != null
				&& f.getFunctionName().getReturn() != null) {
	...
	public Object visit(BBOX filter, Object extraData) {
		return visitBinarySpatialOperator((BinarySpatialOperator) filter,
				extraData);
	}
	...
	protected Object visitBinarySpatialOperator(BinarySpatialOperator filter,
		...
		if (e1 instanceof PropertyName) {
			...
			if (featureType != null) {
				...
				AttributeDescriptor descriptor = (AttributeDescriptor) e1
						.evaluate(featureType);
		...
		if (e1 instanceof PropertyName && e2 instanceof Literal) {
			...
			return visitBinarySpatialOperator(filter, (PropertyName) e1,
					(Literal) e2, filter.getExpression1() instanceof Literal,
					extraData);
		} else {
	...
	/**
	 * Handles the common case of a PropertyName,Literal geometry binary spatial
	 * operator.
	 */
	protected Object visitBinarySpatialOperator(BinarySpatialOperator filter,
	 ...
	 * <p>
	 * The most common case is two PropertyName expressions, which happens
	 * during a spatial join.
	 * </p>
	 */
	protected Object visitBinarySpatialOperator(BinarySpatialOperator filter,
	...
	protected Object visitBinaryTemporalOperator(BinaryTemporalOperator filter,
		...
		if (e1 instanceof PropertyName && e2 instanceof Literal) {
			// call the "regular" method
			return visitBinaryTemporalOperator(filter, (PropertyName) e1,
					(Literal) e2, filter.getExpression1() instanceof Literal,
					extraData);
		} else {
			// call the join version
	 ...
	 */
	protected Object visitBinaryTemporalOperator(BinaryTemporalOperator filter,
			PropertyName property, Literal temporal, boolean swapped,
			Object extraData) {

		Class typeContext = null;
		AttributeDescriptor attType = (AttributeDescriptor) property
				.evaluate(featureType);
		if (attType != null) {
		...
		if ((filter instanceof Begins || filter instanceof BegunBy
				|| filter instanceof Ends || filter instanceof EndedBy
				|| filter instanceof During || filter instanceof TContains)
				&& period == null) {
			if (period == null) {
				throw new IllegalArgumentException(
						"Filter requires a time period");
			}
		}
		if (filter instanceof TEquals && period != null) {
			throw new IllegalArgumentException(
					"TEquals filter does not accept time period");
		}
		...
		if ((filter instanceof Begins || filter instanceof Ends || filter instanceof During)
				&& swapped) {
			throw new IllegalArgumentException(
					"Time period must be second argument of Filter");
		}
		if ((filter instanceof BegunBy || filter instanceof EndedBy || filter instanceof TContains)
				&& !swapped) {
			throw new IllegalArgumentException(
					"Time period must be first argument of Filter");
		}
	...
	void visitBegin(Period p, Object extraData) {
		filterFactory.literal(p.getBeginning().getPosition().getDate()).accept(
				this, extraData);
	}

	void visitEnd(Period p, Object extraData) {
		filterFactory.literal(p.getEnding().getPosition().getDate()).accept(
				this, extraData);
	}
	...
	/**
	 * Encodes a null filter value. The current implementation does exactly
	 * nothing.
	 * 
	 * @param extraData
	 *            extra data to be used to evaluate the filter
	...
	public Object visit(PropertyName expression, Object extraData)
		...
		try {
			...
			try {
				attribute = (AttributeDescriptor) expression
						.evaluate(featureType);
			} catch (Exception e) {
				// just log and fall back on just encoding propertyName straight
				// up
				String msg = "Error occured mapping " + expression
						+ " to feature type";
				LOGGER.log(Level.WARNING, msg, e);
			}
			String encodedField;
			if (attribute != null) {
				encodedField = fieldEncoder.encode(escapeName(attribute
						.getLocalName()));
				if (target != null
						&& target.isAssignableFrom(attribute.getType()
								.getBinding())) {
				...
				encodedField = fieldEncoder.encode(escapeName(expression
						.getPropertyName()));
			}
	 ...
	 *             for io problems
	 */
	protected Object visit(BinaryExpression expression, String operator,
	...
	/**
	 * Writes sql for a function expression. By default it will write the call
	 * by using the same arguments provided to the GeoTools function, subclasses
	 * should override on a case by case basis if this behavior is not the
	 * desired one.
	 * 
	 * @param expression
	...
	public Object visit(Function function, Object extraData)
		...
		try {
			...
			List contexts = null;
			// check context, if a list which patches parameter size list assume
			// its context
			// to pass along to each Expression for encoding
			if (extraData instanceof List
					&& ((List) extraData).size() == parameters.size()) {
			...
			for (int i = 0; i < parameters.size(); i++) {
				...
				Object context = function.getFunctionName().getArguments()
						.get(i).getType();
	...
	/**
	 * Returns the n-th parameter of a function, throwing an exception if the
	 * parameter is not there and has been marked as mandatory
	 * 
	 * @return
	 */
	protected Expression getParameter(Function function, int idx,
			boolean mandatory) {
		...
		if (params == null || params.size() <= idx) {
			if (mandatory) {
				throw new IllegalArgumentException("Missing parameter number "
						+ (idx + 1) + " for function " + function.getName()
	 ...
	 * name in the query should be treated as upper-case and an SQLCODE of -204
	 * or 206 may result if the name is not found.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * Interface for customizing the encoding of Fields, irrespective of which
	 * subclass of FilterToSQL we are using.
	 * 
	 */
